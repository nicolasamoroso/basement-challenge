...
const CartItem = ({ item, index }) => {
  return (
    <li
      key={index}
      className="flex border p-2 sm:p-3 gap-3 mb-3 h-[150px] xs:h-[170px] sm:h-[220px]"
    >
      ...
      <div className="flex flex-col xs:justify-between w-[100%]">
        <div>
          <Text css="font-bold xs:text-[1.2rem] sm:text-[1.5rem] leading-[100%] xs:leading-[100%] md:md-text-shadow pb-2 uppercase">
            {item.product.name}
          </Text>
          <Text css="text-[0.6rem] xs:text-[0.8rem] sm:text-[1rem] text-[#999]">
            {item.product.description}
          </Text>
        </div>
        <div className="flex flex-col gap-2 sm:gap-3 pt-2">
          <div className="flex gap-3 items-center">
            <Text css="text-[0.7rem] xs:text-[1.2rem] sm:text-[1.5rem]">
              QUANTITY:
            </Text>
          ...
          <div className="flex flex-col justify-between xs:items-center xs:flex-row gap-3 sm:gap-3">
            <div className="flex items-center gap-3">
              <Text css="text-[0.7rem] xs:text-[1.2rem] sm:text-[1.5rem] pt-1">
                SIZE:
              </Text>
